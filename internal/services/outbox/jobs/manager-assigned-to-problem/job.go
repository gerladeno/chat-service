...
//go:generate mockgen -source=$GOFILE -destination=mocks/job_mock.gen.go -package=managerassignedtoproblemjobbmocks
...
func (j *Job) Handle(ctx context.Context, payload string) (err error) {
	...
	if len(parts) != 2 {
		return fmt.Errorf("err parsing payload: %s", payload)
	}
