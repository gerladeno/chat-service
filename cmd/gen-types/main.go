...
func run(pkg string, types []string, outFile string) (err error) {
	if len(types) == 0 {
		return nil
	}
	...
	for i := range types {
		...
		builder.WriteString(fmt.Sprintf(idDeclarationAndMethodsTmpl, types[i]))
	}

	file, err := os.Create(outFile)
	if err != nil {
		return fmt.Errorf("creating gen output file: %w", err)
	}
	defer func() {
		_ = file.Sync()
	}()
